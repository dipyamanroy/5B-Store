            ...
            cy.task('isDocker').then((isDocker) => {
                if (!isDocker) {
                    cy.window().then(async () => {
                        const response = await fetch(`${Cypress.config('baseUrl')}/api/Products`, {
                            method: 'POST',
                            cache: 'no-cache',
                            headers: {
    ...
    describe('challenge "changeProduct"', () => {
        it('should be possible to change product via PUT request without being logged in', () => {
            cy.task('GetTamperingProductId').then((tamperingProductId) => {
                const id = tamperingProductId;
                cy.task('GetOverwriteUrl').then((overwriteUrl) => {
                    const url = overwriteUrl;
                    cy.window().then(async () => {
                        const response = await fetch(`${Cypress.config('baseUrl')}/api/Products/${id}`, {
                            method: 'PUT',
                            cache: 'no-cache',
                            headers: {
                                'Content-type': 'application/json'
    ...
    describe('challenge "httpHeaderXss"', () => {
        ...
        it('should be possible to save log-in IP when logged in', () => {
            cy.task('isDocker').then((isDocker) => {
                if (!isDocker) {
                    cy.window().then(async () => {
                        const response = await fetch(`${Cypress.config('baseUrl')}/rest/saveLoginIp`, {
                            method: 'GET',
                            cache: 'no-cache',
                            headers: {
                                Authorization: `Bearer ${localStorage.getItem('token')}`,
